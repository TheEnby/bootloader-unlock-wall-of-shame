...
On some models (such as the Stylo 3 Plus and G6), the bootloader can still be unlocked via `fastboot oem unlock` **only if** [the phone is a T-Mobile model][t-mobile-unlock]. These versions also have most fastboot commands removed, including fastboot flash, erase and boot.
